...
**Typography**: A typography value is an object with the following properties:
- `fontFamily` (string)
- `fontSize` (Dimension)
...
Each section may contain both markdown prose and design tokens in the YAML front matter. The prose elaborates on the tokens' intended use, while the tokens provide exact values.
...
The `colors` section defines all color design tokens. The color tokens should be derived from the key color palettes defined in the markdown prose.
...
Many design systems follow a grid-based layout. Others use margins, safe areas, and dynamic padding.

### Design Tokens

The spacing section defines the spacing design tokens.

It is a map<string, Dimension | number> that maps the spacing scale identifier to a dimension value or a unitless number.
...
This section provides style guidance for component atoms within the design system. Common component types include:
...
The components section defines a collection of design tokens used to ensure consistent styling of common components. It's a map<string, map<string, string>> that maps a component identifier to a group of sub token names and values.

**Variants**: A component may have a variant for different UI states such as active, hover, pressed, etc. Those variant components may be defined under a different but related key.
